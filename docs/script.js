        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                if (searchText) {
                    ...
                    filteredCards = filteredCards.filter(card => card.text.toLowerCase().includes(searchLower) || card.description.toLowerCase().includes(searchLower) || card.name.toLowerCase().includes(searchLower));
                }

                displayCards(filteredCards);

                // Remove the "selected" class from all categories, tags, and cards
                const allLinks = document.querySelectorAll(".menu-item a, .tag-button, .card");
                ...
                if (categoryFilter !== null || searchText) {
                    ...
                    const allTags = data.reduce((acc, card) => [...acc, ...card.tags], []);
                ...
                const selectedLink = document.querySelector(`.menu-item a[data-category="${categoryFilter}"]`);
                if (selectedLink) {
    ...
    function filterByTag(tag) {
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                const selectedLink = document.querySelector(`.tag-button[data-tag="${tag}"]`);
                if (selectedLink) {
                    selectedLink.classList.add("selected");
                }
            })
