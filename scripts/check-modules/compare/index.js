...
const DEFAULT_LEGACY_COMMAND = "node scripts/orchestrator/index.js run --only=check-modules --checks=legacy";
const DEFAULT_TS_COMMAND = "node scripts/orchestrator/index.js run --only=check-modules --checks=ts";
...
function createInitialPlan ({fixturesPath, legacyCommand, tsCommand, runId, runDirectory}) {
  return {
    ...
    runDirectory,
    runs: [],
    notes: ["Harness captures outputs for legacy and TypeScript runs, then diffs the captured outputs."]
  };
}
...
async function writePlanReadme (directory) {
  ...
  const readmeContents = [
    ...
    "",
    "This directory was generated by the comparison harness CLI. It now",
    "captures stdout/stderr alongside selected Stage 5 artifacts for the",
    "legacy and TypeScript runs, then diffs the captured outputs.",
    "",
    "## Generated files",
    "",
    "- `plan.json` — metadata about executed commands and captured artifacts.",
    "- `legacy/`, `ts/` — per-run directories with logs and copied artifacts.",
    "- `diff.json`, `diff.md` — machine-readable and Markdown summaries of comparison results.",
    "",
    "## TODO",
    "",
...
async function runCli () {
  ...
  program
    ...
    .description("Compare outputs between legacy and TypeScript check modules implementations (skeleton stage).")
    ...
    .action(async (options) => {
      ...
      if (hasFailure) {
        ...
        plan.status = "partial-execution";
        notes.push("One or more commands were skipped. Provide commands or rerun to continue toward diffing.");
        plan.diff = {status: "not-run"};
      } else {
